...
class GSOperations(QObject):
    ...
    def run_gs(self, gs_args):
        try:
            # 读取核心样本 ID
            sample_ids = self.get_sample_id(gs_args["core_sample_file"])
            # 读取 PLINK 二进制文件
            genotypes, ids, bim = self.read_plink_bed(gs_args["geno_file"], sample_ids)
            self.progress_signal.emit("基因型数据读取完成")
            # 读取表型数据
    ...
    def get_sample_id(self, sample_file):
        """读取核心样本文件并返回样本 ID"""
    ...
    def read_plink_bed(self, geno_file, sample_ids=None):
        """读取 PLINK 二进制文件（.bed, .bim, .fam）"""
        try:
